 ...
 */
export async function createRoom(roomId, hostName, spawns) {
...
export async function joinRoom(roomId, guestName) {
    ...
    await update(ref(db, "rooms/" + roomId + "/players"), { [slotKey]: guestName });
    
    // Đăng ký onDisconnect tự động xóa slot này khi rớt mạng
    const slotRef = ref(db, "rooms/" + roomId + "/players/" + slotKey);
